...
export async function POST(request: NextRequest) {
  try {
    const { documentId, saveToBlob = false } = await request.json()
    ...
    if (saveToBlob) {
      try {
        // Save to Vercel Blob
        const fileName = `reports/report-${documentId}-${Date.now()}.pdf`
        ...
        return NextResponse.json({
          success: true,
          message: 'Report generated successfully (blob storage unavailable)',
